...
export async function getOptcgCard(
	cardSetId: string,
): Promise<OptcgCardVariant[]> {
	const id = normalizeCardId(cardSetId);
	if (!id) {
		throw new Error("cardSetId must not be empty.");
	}
	const url = `${OPTCG_API_BASE}/sets/card/${encodeURIComponent(id)}/`;
	...
	if (!Array.isArray(data)) {
		throw new Error(data.error || "OPTCG returned no card data.");
	}
	return data.map(toCardVariant);
}

export function toCardVariant(raw: OptcgRawCard): OptcgCardVariant {
	return {
